...
public class FavoriteTeamsService {
	...
	@Autowired
	public FavoriteTeamsService(MessageService messageHelper, Dao dao) {
	...
	public SendMessage favoriteTeams(Long chatId) {
		...
		if (teams.isEmpty())
			return messageHelper.text(Emoji.INFO + " Ваши любимые команды:\n\n<b>У вас пока нет любимых команд!</b> "
					+ Emoji.SAD + "\n\n" + TEAMS_COMMANDS);
		...
		teams.stream()
				.forEach(team -> textMessage.append("<b>").append(team.getName()).append("</b> [")
						...
						.append(messageHelper.flagUnicodeFromCountry(team.getCountryCode().getCode())).append("\n"));
		textMessage.append("\n").append(TEAMS_COMMANDS);
		return messageHelper.text(textMessage);
	}
