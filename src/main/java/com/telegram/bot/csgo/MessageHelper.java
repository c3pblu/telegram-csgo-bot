    ...
    public static SendMessage topTeams(Integer count) {
        ...
        try {
            ...
            for (Element team : rankedTeams) {
                ...
                row.append("<a href=\'https://hltv.org").append(team.select("div.more").select("a[class=details moreLink]").attr("href")).append("\'>")
...
//                textMessage.append("<b>").append(value.select("td.playerCol").text()).append("</b>, ");
                textMessage.append("<a href=\'https://hltv.org").append(value.select("td.playerCol").select("a").attr("href")).append("\'>")
